...
- Users: list (`-userslist`), details (`-user-info`), create (`-fastcreatenopin`, no prefix PIN — TOTP token compatible with Google Authenticator), activate, deactivate, lock, unlock, resync (two consecutive OTP codes), delete, disable prefix PIN requirement (`-set user request_prefix_pin=0`). Before creating, the GUI probes the local SAM (and the joined domain, if any) for the username and warns when no matching Windows account is found.
